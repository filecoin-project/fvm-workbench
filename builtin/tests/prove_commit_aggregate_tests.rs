...
impl PreCommitConfig {
    ...
    pub fn new(commd: Option<Cid>) -> PreCommitConfig {
        PreCommitConfig(SectorDealData { commd })
    }
...
fn valid_precommits_then_aggregate_provecommit() {
    ...
    let (miner_id, _miner) = create_miner(
        &mut w,
        owner.id,
        owner.id,
    ...
    let _duration = expiration - prove_commit_epoch;
    let _deal_spaces = DealSpaces {
