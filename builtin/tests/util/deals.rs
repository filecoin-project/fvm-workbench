use crate::util::{apply_code, apply_ok};
...
pub struct DealBatcher<'w, 'b> {
    w: &'w mut ExecutionWrangler<'b>,
    deals: Vec<DealProposal>,
    default_options: DealOptions,
    published: bool,
}

impl<'w, 'b> DealBatcher<'w, 'b> {
    pub fn new(w: &'w mut ExecutionWrangler<'b>, opts: DealOptions) -> Self {
        DealBatcher { w, deals: vec![], default_options: opts, published: false }
    }
    ...
    pub fn publish_ok(&mut self, sender: Address) -> ExecutionResult {
